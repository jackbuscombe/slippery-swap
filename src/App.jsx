		...
		<Router>
			<Layout style={{ height: "100vh", background: "#f0f2f500" }}>
				<Header style={styles.header}>
					<img src={logo} width="44" height="40" />
					{isAuthenticated && (
						<Menu
							theme="light"
							mode="horizontal"
					...
					<div style={styles.headerRight}>
						...
						<Account metamaskFox={metamaskFox} />
						{isAuthenticated && <Blockie currentWallet size={7} scale={5} />}
					</div>
				</Header>
				<div style={styles.content}>
					<Switch>
						<Route path="/login">
							<Auth logo={logo} metamaskFox={metamaskFox} chain="eth" />
						</Route>
						<Route path="/dex">
							<Dex chain="eth" />
						</Route>
					...
					<Redirect to="/dex" />
					{isAuthenticated ? <Redirect to="/dex" /> : <Redirect to="/login" />}
				</div>
			</Layout>
		</Router>
	);
